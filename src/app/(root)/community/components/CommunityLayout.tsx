...
const CommunityLayout = ({ students, unreadMessagesMap }: CommunityLayoutProps) => {
  ...
  useEffect(() => {
      ...
      const fetchChatData = async () => {
        ...
        } else if (allStudent && mentor){
            const data = await getGroupChat({
                mentorId: mentor?._id,
                studentId: allStudent[0]._id,
  ...
  }, [selectedStudent, mentor, allStudent]);
  ...
  return (
    <>
      <div className="lg:flex px-10 hidden overflow-hidden flex-1 gap-3 justify-center flex-grow items-center">
            ...
            <form>
              <div className="text-left px-4">
                ...
                <div className="flex items-center justify-between mb-1 bg-white rounded-md">
                  <label htmlFor="select-members" className="ml-2">
                    Select Members
                  </label>
                  <input
                    type="checkbox"
        ...
        <ChatContent selectedStudent={selectedStudent} chatData={chatData} allStudents={allStudent}/>
      </div>
