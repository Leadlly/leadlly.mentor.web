  ...
  return (
    <div className="w-full px-3 lg:px-7 space-y-6">
      <div className="w-full flex items-center justify-between">
        <ul className="flex bg-white items-center gap-4">
          {topicsLearntToday.map((item) => (
            <li
              key={item.subject}
              className={cn(
                "capitalize font-semibold text-[#6a6a6a] text-[20px] px-3 py-1 rounded-lg cursor-pointer",
      ...
      <div className="relative" ref={topicModalRef}>
        ...
        {topicModal ? (
          <div className="w-full h-24 absolute inset-x-0 mt-1 z-20 border bg-white text-[black] rounded-lg overflow-y-auto custom__scrollbar">
            {topicsLearntToday.map((item) => (
              <>
                {item.chapters.map(
                  (chapter) =>
