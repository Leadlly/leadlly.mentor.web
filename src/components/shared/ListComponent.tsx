...
import SearchBar from "@/app/(protected)/(root)/(dashboard)/_components/SearchBar";
        ...
        <div className="pt-5 w-full h-[calc(100dvh-120px)] text-center">
          ...
          <div className="overflow-y-scroll custom__scrollbar">
            {students.map((profile: any) => (
              <Link
                href={`/student/${profile._id}`}
                className="flex items-center p-2 px-[25px] gap-6 mb-0.5 bg-white border-b rounded-md min-h-fit"
                key={profile.id}
              >
