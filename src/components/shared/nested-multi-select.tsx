...
interface NestedMultiSelectProps extends VariantProps<
...
>(
  (
    ...
    const toggleOption = (value: Item) => {
      const valueIndex = selectedValues.findIndex((v) => v._id === value._id);
      const newSelectedValues =
        valueIndex >= 0
          ? selectedValues.filter((_, index) => index !== valueIndex)
    ...
    const handleSelectSubTopic = (item: Item, subItem: SubItem) => {
      const valueIndex = selectedValues.findIndex((v) => v._id === item._id);
      ...
      if (valueIndex !== -1) {
        ...
        const value = {
          ...
          subItems: [...(selectedValues[valueIndex].subItems || [])],
        };
        ...
        if (subItemIndex !== -1) {
          ...
          if (value.subItems.length === 0) {
            newSelectedValue = [
              ...selectedValues.slice(0, valueIndex),
              ...selectedValues.slice(valueIndex + 1),
            ];
          } else {
            newSelectedValue = [
              ...selectedValues.slice(0, valueIndex),
